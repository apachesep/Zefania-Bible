...
class BibleReadingPlan
{
	...
	private function fnc_output_seperate($item)
	{
		...
		echo '	  <link>'.JRoute::_(JURI::base()."index.php?option=com_zefaniabible&amp;view=reading&amp;plan=".$item->str_reading_plan."&amp;bible=".$item->str_Bible_Version."&amp;day=".$item->int_day_number).'&amp;ord='.date("mdy").'</link>'.PHP_EOL;
		echo '	</image>'.PHP_EOL;		
		echo '	<title>'.JText::_('ZEFANIABIBLE_READING_RSS_TITLE') .' - '. $item->str_reading_plan_name.  " - ". $item->str_bible_name. '</title>'.PHP_EOL;
		echo '	<link>'.JRoute::_(JURI::base()."index.php?option=com_zefaniabible&amp;view=reading&amp;plan=".$item->str_reading_plan."&amp;bible=".$item->str_Bible_Version."&amp;day=".$item->int_day_number).'&amp;ord='.date("mdy").'</link>'.PHP_EOL;				
		...
		foreach($item->arr_plan as $reading)
		{
			...
			{					
				if (($plan->book_id > $book)or($plan->chapter_id > $chap))
				{
					...
					echo '		<link>'.JRoute::_(JURI::base()."index.php?option=com_zefaniabible&amp;view=reading&amp;plan=".$item->str_reading_plan."&amp;bible=".$item->str_Bible_Version."&amp;day=".$item->int_day_number).'&amp;ord='.date("mdy").'#'.$y.'</link>'.PHP_EOL;	
					echo '		<guid>'.JRoute::_(JURI::base()."index.php?option=com_zefaniabible&amp;view=reading&amp;plan=".$item->str_reading_plan."&amp;bible=".$item->str_Bible_Version."&amp;day=".$item->int_day_number).'&amp;ord='.date("mdy").'#'.$y.'</guid>'.PHP_EOL;
	...
	private function fnc_output_single($item)
	{
		...
		echo '		<link>'.JRoute::_(JURI::base()."index.php?option=com_zefaniabible&amp;view=reading&amp;plan=".$item->str_reading_plan."&amp;bible=".$item->str_Bible_Version."&amp;day=".$item->int_day_number).'&amp;ord='.date("mdy").'</link>'.PHP_EOL;	
		echo '		<guid>'.JRoute::_(JURI::base()."index.php?option=com_zefaniabible&amp;view=reading&amp;plan=".$item->str_reading_plan."&amp;bible=".$item->str_Bible_Version."&amp;day=".$item->int_day_number).'&amp;ord='.date("mdy").'</guid>'.PHP_EOL;
		echo '		<pubDate>'.date('D, d M Y 00:00:00').'</pubDate>'.PHP_EOL;		
		echo '		<description>';	
		echo '		'.$str_desc;
		echo '		</description>'.PHP_EOL;
