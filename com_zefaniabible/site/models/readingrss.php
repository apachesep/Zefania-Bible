 ...
 *
 */
class ZefaniabibleModelReadingrss extends ZefaniabibleModelList
{
	var $_name_sing = 'readingrssitem';
	/**
	 * Constructor
	 *
	 */
	...
	function _buildQuery_Bibles()
	{
		...
		{
			$db = $this->getDbo();
			$query =  'SELECT a.alias, a.bible_name, a.xml_audio_url FROM `#__zefaniabible_bible_names` AS a WHERE publish = 1 ORDER BY a.bible_name';
			$db->setQuery($query);
			$data = $db->loadObjectList();	
		}
		catch (JException $e)
