...
{
	var $ctrl = 'zefaniareading';
	var $singular = 'zefaniareadingitem';
	...
	function new_()
	{
		...
		//Description
		$filter_description = $app->getUserState( $this->context . ".filter.description");
		if ($filter_description) $vars["filter_description"] = $filter_description;
	...
	function edit()
	{
		...
		$model = $this->getModel('zefaniareadingitem');
		$item = $model->getItem();
	...
	function delete()
	{
		...
		if (parent::_delete($cid))
		{
			...
			{
				case 'default':
					JRequest::setVar( 'view'  , 'zefaniareading');
	...
	function toggle_publish()
	{
		...
		$data = array("publish" => is_null($zefaniareadingitem->publish)?1:!$zefaniareadingitem->publish);
        $this->_save($data);
