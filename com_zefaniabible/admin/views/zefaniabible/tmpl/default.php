...
<form action="<?php echo JRoute::_('index.php?option=com_zefaniabible&view=zefaniabible'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php endif;?>
	...
	<table class="table table-striped" id="zefaniabible_bible_namesList">
		<thead>
			<tr>
		...
		$canEdit	= $user->authorise('core.edit',       'com_zefaniabible.zefaniabible_bible_names.'.$item->id);
		$canCheckin	= $user->authorise('core.manage',     'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
		$canEditOwn	= $user->authorise('core.edit.own',   'com_zefaniabible.zefaniabible_bible_names.'.$item->id) && $item->created_by == $userId;
		$canChange	= $user->authorise('core.edit.state', 'com_zefaniabible.zefaniabible_bible_names.'.$item->id) && $canCheckin;
		?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->published)): ?>
					<td class="center">
						<?php echo JHtml::_('jgrid.published', $item->published, $i, 'zefaniabible.', $canChange, 'cb'); ?>
					</td>
                <?php endif; ?>
				...
				<td class="nowrap has-context">
						<div class="pull-left">
							<?php if ($item->checked_out) : ?>
								<?php echo JHtml::_('jgrid.checkedout', $i, null, $item->checked_out_time, 'zefaniabible.', $canCheckin); ?>
							<?php endif; ?>
							<?php if ($canEdit || $canEditOwn) : ?>
								<a href="<?php echo JRoute::_('index.php?option=com_zefaniabible&task=zefaniabibleitem.edit&id='.(int) $item->id); ?>">
								<?php echo $this->escape($item->bible_name); ?></a>
							<?php else : ?>
								<?php echo $this->escape($item->bible_name); ?>
							<?php endif; ?>
						</div>
						<div class="pull-left">
							<?php
								// Create dropdown items
								JHtml::_('dropdown.edit', $item->id, 'zefaniabibleitem.');
								if (!isset($this->items[0]->published) || $this->state->get('filter.published') == -2) :
									JHtml::_('dropdown.addCustomItem', JText::_('JTOOLBAR_DELETE'), 'javascript:void(0)', "onclick=\"contextAction('cb$i', 'zefaniabible.delete')\"");
								endif;
								JHtml::_('dropdown.divider');

								if ($item->published != 0) :
									JHtml::_('dropdown.unpublish', 'cb' . $i, 'zefaniabible.');
								endif;
								if ($item->published != 1) :
