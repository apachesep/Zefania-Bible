...
*/
	...
	function attachJsFiles()
	{
		...
		{
			$relativeName = $fileBase . $jsFileName;
			$this->addScript($relativeName);
		}
	}
	 ...
	 * 					ie : "<%name%> <%surname%> <%_user_email%>"
	 * 					note : theses values should be available in $object
	 */
	function parseKeys($object, $pattern)
	{
		...
		$matches = array();
		if (preg_match_all("/<%([a-zA-Z0-9_]+:)?([a-zA-Z0-9_]+)%>/", $pattern, $matches))
		{
			...
			foreach($matches[0] as $match)
			{
				...
				$label = preg_replace("/<%([a-zA-Z0-9_]+:)?" . $key . "%>/", $value, $label);
				$index++;
