...
/**
 * List Model for zefaniacomment.
 *
 * @package     Zefaniabible
 * @subpackage  Models
 */
class ZefaniabibleModelZefaniacomment extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'a.ordering', 'ordering',
				'a.language', 'language','ordering', 'state', 'title', 'full_name'
			);
		}
	...
	protected function populateState($ordering = 'title', $direction = 'ASC')
	{
		...
		$this->setState('filter.title', $title);
				// Set filter state for full_name
		$full_name = $this->getUserStateFromRequest($this->context.'.filter.full_name', 'filter_full_name', '');
		$this->setState('filter.full_name', $full_name);
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__zefaniabible_zefaniacomment AS a');				
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('a.title LIKE' . $s . ' OR a.full_name LIKE' . $s );
			}
		}
		...
		if ($title != "")
		{
			$query->where('a.title = ' . $db->quote($db->escape($title)));
		}

		// Filter by full_name
		$full_name = $this->getState('filter.full_name');
		if ($full_name != "")
		{
			$query->where('a.full_name = ' . $db->quote($db->escape($full_name)));
		}
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__zefaniabible_zefaniacomment AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
