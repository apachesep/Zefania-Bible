...
/**
 * List Model for zefaniacommentdetail.
 *
 * @package     Zefaniabible
 * @subpackage  Models
 */
class ZefaniabibleModelZefaniacommentdetail extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__zefaniabible_comment_text AS a');				
		...
		if (!empty($search))
		{
			...
			elseif (stripos($search, 'bible_id:') === 0)
			{
				$search = $db->quote('%' . $db->escape(substr($search, strlen('bible_id:')), true) . '%');
				$query->where('(a.bible_id LIKE ' . $search);
			}
			else
			{
				$search = $db->quote('%' . $db->escape($search, true) . '%');
				$query->where('a.bible_id LIKE' . $s . ' OR a.book_id LIKE' . $s . ' OR a.chapter_id LIKE' . $s . ' OR a.verse_id LIKE' . $s );
			}
		}
		// Filter by bible_id
