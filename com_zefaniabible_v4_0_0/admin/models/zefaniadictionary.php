...
/**
 * List Model for zefaniadictionary.
 *
 * @package     Zefaniabible
 * @subpackage  Models
 */
class ZefaniabibleModelZefaniadictionary extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$query->select('a.*')->from('#__zefaniabible_dictionary_info AS a');				
	...
	public function getAuthors()
	{
		...
		$query->select('u.id AS value, u.name AS text')
			->from('#__users AS u')
			->join('INNER', '#__zefaniabible_dictionary_info AS a ON a.created_by = u.id')
			->group('u.id, u.name')
			->order('u.name');
