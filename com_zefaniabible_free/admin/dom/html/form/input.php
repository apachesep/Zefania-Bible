...
class JDomHtmlFormInput extends JDomHtmlForm
{
	...
	var $validatorInsensitive = false;
	var $validatorInvert = false;
	var $validatorModifiers = '';
	var $dateFormat;
	var $domName;
	var $formControl;
	...
	function addValidatorHandler($regex= null, $handler = null)
	{
		...
		if (defined('JQUERY') && (JQUERY == 'BETA'))
		{
			...
			$script = '$.validationEngineLanguage.allRules.' . $this->validatorHandler . ' = ' . $jsRule;
	...
	function buildValidatorMessage()
	{
		if (defined('JQUERY'))
		{
			if (JQUERY == 'BETA')
			{
				// Dynamically instance the JS validators rules
				// This is a temporally insertion, before the coming of V2.0
				// You will find this code in JForms
	...
	*/
	public function getJsonRule()
	{
		...
		$values = array(
			...
			"alertText" => LI_PREFIX . JText::_($this->validatorMsgIncorrect)
		);
