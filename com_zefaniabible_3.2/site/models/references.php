 ...
 *
 */
class ZefaniabibleModelReferences extends ZefaniabibleModelList
{
	var $_name_sing = 'referencesitem';
	/**
	 * Constructor
	 *
	 */
	...
	public function _buildQuery_References($str_bible_version, $int_Bible_Book_ID, $int_Bible_Chapter, $int_Bible_Verse )
	{
		try 
		{
			$db = $this->getDbo();
	...
	public function fnc_make_verse($str_Bible_Version,$int_book_id,$int_bible_chapter,$str_start_verse,$str_end_verse)
	{
		try 
		{
			...
			$query->where("b.alias='".trim($str_Bible_Version)."'");
			$query->where("a.book_id=".(int)$int_book_id);
			$query->where("a.chapter_id=".$int_bible_chapter);
			if($str_end_verse == 0)
			{
